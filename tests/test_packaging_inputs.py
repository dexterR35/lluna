...
def test_packaging_inputs_validate_without_building():
    subprocess.run([sys.executable,"packaging/build.py","--validate-only"],check=True)
...
def test_forge_embeds_only_the_frozen_sidecar():
    ...
    assert 'rename(path.join(resourcesPath, "midgard-backend"), path.join(resourcesPath, "backend-sidecar"))' in source
