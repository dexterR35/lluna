...
def test_source_paths_remain_inside_checkout(monkeypatch, tmp_path: Path) -> None:
    ...
    root.mkdir()
    monkeypatch.setenv("LLUNA_PROJECT_ROOT", str(root))
...
def test_frozen_linux_paths_are_user_writable(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("sys.executable", str(bundle / "Lluna"))
    ...
    assert paths.config_dir == config_home / "lluna"
    assert paths.data_dir == data_home / "lluna"
    assert paths.models_dir == data_home / "lluna" / "models"
    assert paths.updates_dir == data_home / "lluna" / "updates"
