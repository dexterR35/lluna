...
Grounding DINO; FLUX, SDXL Turbo, and Stable Diffusion 1.5; download registry
...
|---|---|---|---|---|---|---|---|
| `FLUX.2-klein-4B` | FLUX.2 Klein 4B; fast text-to-image | Diffusers/Transformers/PyTorch; `black-forest-labs/FLUX.2-klein-4B` | Apache-2.0; current model card describes open commercial use; not marked gated | Diffusers `model_index.json`, scheduler, transformer, text encoder(s), tokenizer(s), VAE, one supported weight format each, marker; `backend/models/generate/FLUX.2-klein-4B/` | Manifest absent; snapshot unpinned. Current local directory is about 23 GiB and includes redundant top-level/component weights | Midgard has no enforced minimum; upstream card indicates about 13 GB VRAM. CUDA only, BF16 or FP16 | Catalog default but **nothing is enabled by default**; install enables it. One generation pipeline cached; switching releases previous; CPU offload configured; explicit release deletes and clears CUDA |
| `FLUX.2-klein-9B` | FLUX.2 Klein 9B; higher-capacity text-to-image | Same stack; `black-forest-labs/FLUX.2-klein-9B` | FLUX non-commercial license; gated with license/contact acceptance | Equivalent Diffusers snapshot at `.../FLUX.2-klein-9B/` | Unknown/unverified/unpinned; absent here | No Midgard minimum; upstream card indicates about 29 GB VRAM. CUDA only, BF16/FP16 | Optional; same one-entry cache |
| `SDXL-Turbo` | SDXL Turbo; low-step text-to-image | Diffusers/Transformers/PyTorch; `stabilityai/sdxl-turbo` | Current Hugging Face metadata uses Stability AI Community License terms; repo access currently not treated as gated by Midgard | Equivalent snapshot at `.../SDXL-Turbo/` | Unknown/unverified/unpinned; absent here | No declared/enforced minimum; CUDA only, BF16/FP16 | Optional; same cache |
| `SD-1.5` | Stable Diffusion 1.5; general text-to-image | Diffusers/Transformers/PyTorch; `runwayml/stable-diffusion-v1-5` | Catalog assumes the historical source/license, but the configured endpoint was not anonymously accessible during this audit; gated/availability must be revalidated | Equivalent snapshot at `.../SD-1.5/` | Unknown/unverified/unpinned; absent here | No declared/enforced minimum; CUDA only, BF16/FP16 | Optional; same cache |
