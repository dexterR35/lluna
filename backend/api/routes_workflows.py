...
def validate(workflow: WorkflowDocument) -> dict:
    return validate_workflow(workflow, include_unrelated=True, check_model_availability=True).model_dump(mode="json", by_alias=True)


@router.post("/compile")
def compile(workflow: WorkflowDocument) -> dict:
    return compile_workflow(workflow, check_model_availability=True).model_dump(mode="json", by_alias=True)
