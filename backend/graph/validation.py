    ...
    if mode in {"from", "selected"} and not active:
        issues.append(
            ValidationIssue(
                severity="error",
                code="NO_SELECTION",
                message="Select a node before running from here.",
            )
        )
    duplicate_node_ids = {
        node_id
        for node_id, count in Counter(node.id for node in workflow.nodes).items()
        if count > 1
    }
    if duplicate_node_ids & validation_ids:
        issues.append(
            ValidationIssue(
                severity="error", code="DUPLICATE_NODE_ID", message="Node IDs must be unique."
            )
        )
        ...
        if definition is None:
            issues.append(
                ValidationIssue(
                    severity="error",
                    code="UNKNOWN_NODE",
                    message=f"Unknown node type: {node.schema_id}",
                    node_id=node.id,
                    action="Remove or migrate this node.",
                )
            )
            continue
        if node.schema_version != definition.schema_version:
            issues.append(
                ValidationIssue(
                    severity="error",
                    code="NODE_VERSION",
                    message=f"Unsupported schema version for {definition.name}.",
                    node_id=node.id,
                    action="Migrate this workflow.",
                )
            )
        if not definition.available:
            issues.append(
                ValidationIssue(
                    ...
                    message=definition.unavailable_reason or f"{definition.name} is unavailable.",
                    node_id=node.id,
                )
            )
        if node.disabled:
                ...
                if parameter.minimum is not None and value < parameter.minimum:
                    issues.append(
                        ValidationIssue(
                            ...
                            message=f"{parameter.label} is below {parameter.minimum}.",
                            node_id=node.id,
                        )
                    )
                if parameter.maximum is not None and value > parameter.maximum:
                    issues.append(
                        ValidationIssue(
                            severity="error",
                            code="PARAMETER_RANGE",
                            message=f"{parameter.label} is above {parameter.maximum}.",
                            node_id=node.id,
                        )
                    )
        ...
        if check_model_availability and os.environ.get("LLUNA_FAKE_WORKER") != "1":
            ...
            if model_parameter:
                ...
                selected_option = next(
                    (
                        item
                        for item in model_parameter.options
        ...
        if source_port is None or target_port is None:
            issues.append(
                ValidationIssue(
                    severity="error",
                    code="UNKNOWN_PORT",
                    message="Connection references an unknown port.",
                    edge_id=edge.id,
                )
            )
            continue
        if not ports_compatible(PortType(source_port.type), PortType(target_port.type)):
            issues.append(
                ValidationIssue(
                    ...
                    message=f"{source_port.label} ({source_port.type}) cannot connect to {target_port.label} ({target_port.type}).",
                    edge_id=edge.id,
                )
            )
        ...
        if connected_inputs[key] > 1 and not target_port.multiple:
            issues.append(
                ValidationIssue(
                    ...
                    message=f"{target_port.label} accepts only one connection.",
                    node_id=target.id,
                    edge_id=edge.id,
                )
            )
            ...
            if input_port.required and not connected_inputs[(node.id, input_port.id)]:
                issues.append(
                    ValidationIssue(
                        severity="error",
                        code="MISSING_INPUT",
                        message=f"{input_port.label} is required.",
                        node_id=node.id,
                        action="Connect a compatible output.",
                    )
                )
    ...
    if len(ordered) != len(validation_ids):
        issues.append(
            ValidationIssue(
                ...
                action="Remove the feedback connection.",
            )
        )
    else:
