...
_NODES = [
    ...
    node("midgard.input.images", "Load Images", "Input/Media", "Loads an ordered image queue through desktop file grants.", kind="input", icon="images", outputs=[port("images", "Images", PortType.IMAGE, multiple=True)], parameters=[parameter("pathGrantIds", "Image files", "files", [], required=True)], adapter="load_images"),
    ...
    node("midgard.image.upscale", "Upscale Image", "Image/Enhance", "Upscales one image or an ordered image queue with Real-ESRGAN.", icon="zoom-in", inputs=[port("image", "Image queue", PortType.IMAGE, required=True, multiple=True)], outputs=[port("image", "Images", PortType.IMAGE, multiple=True)], parameters=[parameter("model", "Model", "model", "RealESRGAN_x2plus", options=UPSCALE_MODELS), parameter("denoise", "Pre-denoise", "boolean", False), parameter("denoiseStrength", "Denoise strength", "select", "safe", options=[{"value": "safe", "label": "Safe"}, {"value": "medium", "label": "Medium"}]), parameter("maxLongEdge", "Maximum long edge", "integer", 5000, minimum=256, maximum=32768, description="Caps the final image size to manage memory.")], capabilities=["pytorch"], required_models=["realesrgan-x2"], supports_preview=True, adapter="enhance"),
    node("midgard.image.remove_background", "Remove Background", "Image/Remove", "Produces transparent cutouts from one image or an ordered image queue.", icon="scissors", inputs=[port("image", "Image queue", PortType.IMAGE, required=True, multiple=True), port("protectMask", "Protect Mask", PortType.MASK)], outputs=[port("image", "Cutouts", PortType.IMAGE, multiple=True), port("alpha", "Alpha", PortType.ALPHA, multiple=True)], parameters=[parameter("model", "Model", "model", "birefnet-general", options=BACKGROUND_MODELS)], capabilities=["onnx"], required_models=["rembg"], supports_preview=True, adapter="bg_remove"),
    node("midgard.image.low_light", "Fix Low Light", "Image/Enhance", "Restores one dark image or an ordered image queue with MIRNet.", icon="sun", inputs=[port("image", "Image queue", PortType.IMAGE, required=True, multiple=True)], outputs=[port("image", "Images", PortType.IMAGE, multiple=True)], parameters=[parameter("model", "Model", "model", "MIRNet_LOL", options=[option("MIRNet_LOL", "MIRNet LOL", "mirnet", "Low-light image restoration.")])], required_models=["mirnet"], supports_preview=True, adapter="low_light"),
    node("midgard.image.composite", "Composite Background", "Image/Compose", "Places transparent foregrounds over generated or uploaded backgrounds.", icon="layers", inputs=[port("foreground", "Foregrounds", PortType.IMAGE, required=True, multiple=True), port("background", "Backgrounds", PortType.IMAGE, required=True, multiple=True)], outputs=[port("image", "Composites", PortType.IMAGE, multiple=True)], parameters=[parameter("fit", "Background fit", "select", "cover", options=[{"value": "cover", "label": "Cover"}, {"value": "contain", "label": "Contain"}, {"value": "stretch", "label": "Stretch"}])], supports_preview=True, adapter="composite"),
    ...
    node("midgard.output.preview_image", "Preview Image", "Output/Preview", "Shows one image or an ordered image queue on the node.", kind="output", icon="eye", inputs=[port("image", "Images", PortType.IMAGE, required=True, multiple=True)], outputs=[port("image", "Images", PortType.IMAGE, multiple=True)], supports_preview=True, adapter="passthrough"),
