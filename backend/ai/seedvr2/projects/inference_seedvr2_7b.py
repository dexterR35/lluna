...
def configure_runner(sp_size, checkpoint_dir=None):
    ...
    config_path = SOURCE_ROOT / 'configs_7b' / 'main.yaml'
    ...
    runner.configure_dit_model(device="cuda", checkpoint=str(checkpoint_dir / 'seedvr2_ema_7b.pth'))
...
def generation_loop(runner, video_path='./test_videos', output_dir='./results', batch_size=1, cfg_scale=1.0, cfg_rescale=0.0, sample_steps=1, seed=666, res_h=1280, res_w=720, sp_size=1, out_fps=None):
    ...
    def _extract_text_embeds():
        ...
        for texts_pos in tqdm(original_videos_local):
            text_pos_embeds = torch.load(SOURCE_ROOT / 'pos_emb.pt')
            text_neg_embeds = torch.load(SOURCE_ROOT / 'neg_emb.pt')
