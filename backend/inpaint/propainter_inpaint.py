...
def read_mask(mpath, length, size, flow_mask_dilates=8, mask_dilates=5):
    ...
    masks_dilated = []
    flow_masks = []
    # If a numpy array is passed directly
    if isinstance(mpath, np.ndarray):
        if mpath.ndim == 3 and mpath.shape[2] == 1:
            mpath = mpath.squeeze(2)  # From (H,W,1) to (H,W)
...
class PropainterInpaint:
    ...
    def __call__(self, input_frames: List[np.ndarray], input_mask: np.ndarray):
        ...
        for j in range(len(frames_hr)):
            ...
            for k in range(len(inpaint_area)):
                image_crop = image[inpaint_area[k][0]:inpaint_area[k][1], inpaint_area[k][2]:inpaint_area[k][3], :]  # Crop
                mask_crop = mask[inpaint_area[k][0]:inpaint_area[k][1], inpaint_area[k][2]:inpaint_area[k][3], :]  # Crop
                frames_scaled[k].append(image_crop)  # Append cropped frame
                masks_scaled[k].append(mask_crop)  # Append cropped mask
        ...
        if inpaint_area:
            for j in range(len(frames_hr)):
                ...
                for k in range(len(inpaint_area)):
                    comp = comps[k][j]  # Inpainted frame
