...
test.beforeAll(async () => {
  electronApp = await electron.launch({
    args: [path.resolve(__dirname, "../.e2e/build/main.js")],
...
test("Electron enforces production renderer security", async () => {
  ...
  expect(mainSource).not.toContain("app.commandLine.appendSwitch");
});
...
test("Electron starts its control plane and renders the backend-owned node catalog", async () => {
  ...
  await expect(
    page.locator('[title^="Drag Load Image onto the canvas"]'),
  ).toBeVisible();
  await expect(
  ...
  await expect(
    page.getByRole("button", { name: /^Show Metadata\b/ }),
  ).toHaveCount(0);
  await expect(
    page.getByRole("button", { name: /^Workflow Note\b/ }),
  ).toHaveCount(0);
  await expect(page.getByText("Backend ready", { exact: true })).toBeVisible();

  const canvas = page.locator(".react-flow");
  await page
    .locator('[title^="Drag Upscale Image onto the canvas"]')
    .dragTo(canvas, { targetPosition: { x: 420, y: 300 } });
  const upscaleNode = page.getByLabel("Upscale Image node");
  await expect(
    upscaleNode.getByRole("button", { name: "Run from here Upscale Image" }),
  ).toBeVisible();
  ...
  await expect
    .poll(async () =>
      Number(
        (await viewport.getAttribute("style")).match(/scale\(([^)]+)\)/)?.[1],
      ),
    )
    .toBeGreaterThan(0.85);
  const nodeBoxAfterZoom = await upscaleNode.boundingBox();
  expect(
    Math.abs(nodeBoxAfterZoom.x + nodeBoxAfterZoom.width / 2 - zoomPoint.x),
  ).toBeLessThan(3);
  expect(
    Math.abs(nodeBoxAfterZoom.y + nodeBoxAfterZoom.height / 2 - zoomPoint.y),
  ).toBeLessThan(3);
  const scaleBeforeDoubleClick = (await viewport.getAttribute("style")).match(
    /scale\(([^)]+)\)/,
  )?.[1];
  await upscaleNode.dblclick();
  await page.waitForTimeout(350);
  const scaleAfterDoubleClick = (await viewport.getAttribute("style")).match(
    /scale\(([^)]+)\)/,
  )?.[1];
  ...
  await expect(page.getByRole("dialog", { name: "Upscale Image" })).toHaveCount(
    0,
  );
  await upscaleNode
    .getByRole("button", { name: "Open Upscale Image options" })
    .click();
  const nodeEditor = page.getByRole("dialog", { name: "Upscale Image" });
  await expect(
    nodeEditor.getByRole("button", { name: /Real-ESRGAN ×2/ }),
  ).toBeVisible();
  await nodeEditor.getByRole("button", { name: /Real-ESRGAN ×4/ }).click();
  await nodeEditor.getByRole("button", { name: "Done" }).click();
  await expect(
    page.getByLabel("Upscale Image node").getByText("Real-ESRGAN ×4"),
  ).toBeVisible();
});
