...
if %errorlevel% equ 0 (
  ...
  for /f "tokens=1" %%V in ('py -0 2^>nul') do (
    ...
    if !errorlevel! equ 0 set "LLUNA_PY_TAG=%%V"
  )
  if defined LLUNA_PY_TAG goto run_py_tag
)
...
where uv >nul 2>nul
if %errorlevel% equ 0 (
  for /f "usebackq delims=" %%P in (`uv python find 3.12 2^>nul`) do set "LLUNA_UV_PYTHON=%%P"
)
if defined LLUNA_UV_PYTHON (
  "%LLUNA_UV_PYTHON%" -c "import sys; raise SystemExit(0 if sys.version_info[:2] == (3, 12) else 1)" >nul 2>nul
